...
def test_user_voted_true(mongodb):
    assert user_voted(mongodb, "images\\cat_three.jpg", 474757310) is True


def test_user_voted_false(mongodb):
    assert user_voted(mongodb, "images\\cat_three.jpg", 13165) is False


def test_get_image_rating(mongodb):
    assert get_image_rating(mongodb, "images\\cat_three.jpg") == 1
    assert get_image_rating(mongodb, "images\\no_image.jpg") == 0
